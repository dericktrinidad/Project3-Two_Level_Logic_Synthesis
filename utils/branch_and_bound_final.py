...
class BB_tree:
    ...
    def MiS_quick(self, matrix:np.array):
        ...
        while matrix.size > 0 and matrix.shape[0] > 0:
            row_sums = np.sum(matrix, axis = 1)
            min_sum = min(row_sums)
            min_row_indices = np.where(row_sums == min_sum)[0]
            
            best_rows = []
            for row_idx in min_row_indices:
                delete_rows = {row_idx}
                col_indices = np.where(matrix[row_idx, :] == 1)[0]
                for col_idx in col_indices:
                    row = matrix[:, col_idx]
                    row_indices = np.where(row == 1)[0]
                    delete_rows.update(set(row_indices))
    ...
    def choose_Pi(self, matrix:np.array):
        col_zi = []
        ...
        selected_minterms = [row_idx for row_idx, weight in enumerate(row_weights) if weight == max_weight]
        return random.choice(selected_minterms)
    ...
    def split_logic_matrix(self, matrix: minterm_matrix, Pi, curr_equation):
        ...
        Pi_row = curr_matrix[Pi, :]
        S1_indices = np.where(Pi_row == 1)[0]
        S1_equation = [curr_minterms[Pi]]
        S1_equation.extend(curr_equation)
        S1_matrix = np.delete(next_matrix, S1_indices, axis=1)
        S1_prime_implicants = np.delete(curr_prime_implicants, S1_indices, axis=0)
        if S1_matrix.size > 0:
            S1_minterms = next_minterms
        else:
            S1_minterms = []
        S1_node = minterm_matrix(S1_matrix, S1_prime_implicants, S1_minterms)
        S0_indices = np.where(Pi_row == 0)[0]
        S0_equation = []
        S0_equation.extend(curr_equation)
        S0_matrix = np.delete(next_matrix, S0_indices, axis=1)
        S0_prime_implicants = np.delete(curr_prime_implicants, S0_indices, axis=0)
        if S0_matrix.size > 0:
            S0_minterms = next_minterms
        else:
            S0_minterms = []
        S0_node = minterm_matrix(S0_matrix, S0_prime_implicants, S0_minterms)
    ...
    def reduce_matrix(self, curr_matrix:np.array, curr_prime_implicants:np.array, curr_minterms:np.array):
        ...
        col_to_delete = set(essential_column_idx)
        for row_idx in essential_minterms_idxs:
            ess_row = curr_matrix[row_idx, :]
            col_to_delete.update(np.where(ess_row == 1)[0])
        next_matrix = np.delete(curr_matrix, list(col_to_delete), axis=1)
        next_prime_implicants = np.delete(curr_prime_implicants, list(col_to_delete), axis=0)
    ...
    def minterm_dominance(self, curr_matrix:np.array, curr_prime_implicants:list, curr_minterms:list):
        ...
        for i in range(curr_matrix.shape[0]):
            # row_i = curr_matrix[i, :]
            # row_i_tuple = tuple(sorted(row_i))
            # if row_i_tuple in unique_rows:
